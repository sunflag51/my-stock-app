...
# 【魔法のコード】matplotlibがない場合は、プログラム自身が自動でインストールする
try:
    ...
    if not ticker_symbol:
        ...
        with st.spinner(f"データを取得・分析中..."):
            ...
            if df.empty or len(df) < 80:
                ...
                st.markdown("---")
                if company_name:
                    st.subheader(f"🏢 {company_name}【{ticker_symbol}】の基本情報 (ファンダメンタル)")
                else:
                ...
                title_name = f"{company_name} ({ticker_symbol})" if company_name else ticker_symbol
                ax1.set_title(f"{title_name} - Technical Dashboard ({display_period})", fontsize=14)
                ...
                # --- 💡 修正箇所：KDJの判定テキスト ---
                kdj_mark = "🟢" if (kdj_golden or kdj_above) else "❌"
                st.markdown(f"**【KDJ チェック】** {kdj_mark} (K: {latest_k:.1f} / D: {latest_d:.1f} / J: {latest_j:.1f})")
                st.markdown("→ " + ('KがDを上抜け、買いシグナル（ゴールデンクロス）を形成しています！' if kdj_golden else ('KがDより上にあり、上昇トレンドが継続しています。' if kdj_above else 'KDJのモメンタムは弱め（または下落トレンド）です。')))
