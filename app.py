...
with col1:
    ticker_choice = st.selectbox(
        "銘柄選択:", 
        ["KO (コカ・コーラ)", "V (ビザ)", "AAPL (アップル)", "ISRG (インテュイティブ)", "COST (コストコ)", "その他（手入力）"]
    )
    if ticker_choice == "その他（手入力）":
        user_input = st.text_input("コード（または登録した企業名）を入力:", value="7974.T").strip()
        
        # 💡 Googleスプレッドシート（エクセル表）から自動で読み込む処理
        # ↓↓↓ 以下の " " の中に、さきほどコピーした共有リンクを貼り付けてください ↓↓↓
        sheet_link = "ここにコピーしたスプレッドシートのリンクを貼り付ける"
        
        name_to_code = {}
        if sheet_link.startswith("http"):
            try:
                # 共有リンクをアプリが読めるデータ形式に自動変換
                csv_url = sheet_link.split("/edit")[0] + "/export?format=csv"
                # すでに使っているpandas(pd)の機能で表を一括読み込み
                ...
                st.warning("表の読み込みに失敗しました。リンクと共有設定を確認してください。")
        
        # 入力された文字が表にあればコードに変換、なければそのまま使う
        ticker_symbol = name_to_code.get(user_input, user_input).upper()
