    ...
    if df.empty:
        ...
        for index, row in df.iterrows():
            ...
            if current_price > 0 and qty > 0:
                ...
                # --- マイナス表示のバグ修正部分 ---
                if pnl_amount > 0:
                    delta_str = f"+{currency}{pnl_amount:,.2f}"
                elif pnl_amount < 0:
                    delta_str = f"-{currency}{abs(pnl_amount):,.2f}"
                else:
                    delta_str = f"{currency}0.00"
                ...
                with st.container():
                    ...
                    c3.metric("現在の評価額", f"{currency}{current_amount:,.2f}", delta_str)
