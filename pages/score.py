    ...
    if not ticker_symbol:
        ...
        with st.spinner(f"データを取得・分析中..."):
            ...
            if df.empty or len(df) < 80:
                ...
                ax1.set_title(f"{ticker_symbol} - Technical Dashboard", fontsize=14)
                ...
                st.markdown("以下の項目をスライダーで評価してください。自動判定スコアと合算し、100点満点で評価します。")
                ...
                with st.expander("詳細なテクニカル自動採点の内訳を見る"):
                    st.write(f"- ③ セリングクライマックス判定: **{score_3}/12**")
                    st.write(f"- ④ ボリンジャーバンド(-2σ)判定: **{score_4}/10**")
                    st.write(f"- ⑤ RSI/MACD/KDJ 勢い判定: **{score_5}/12**")
                    st.write(f"- ⑥ ローソク足底型判定: **{score_6}/10**")
                    st.write(f"- ⑦ ダウ理論(安値切り上げ)判定: **{score_7}/16**")
                    st.write(f"- ⑧ 出来高/OBV(資金流入)判定: **{score_8}/10**")
                    st.write(f"- ⑨ 移動平均線(短期/中期)判定: **{score_9}/10**")
