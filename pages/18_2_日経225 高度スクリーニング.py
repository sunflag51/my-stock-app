  ...
  try:
    ...
    for t in tables:
      ...
      if any("証券コード" in c or "コード" in c for c in cols) and any(
        ...
        code_col = [c for c in cols if "コード" in c][0]
        name_col = [c for c in cols if "銘柄" in c][0]
        sub_df = t[[code_col, name_col]].copy()
        sub_df.columns = ["code", "Security"]
        dfs.append(sub_df)

    if dfs:
      df = pd.concat(dfs, ignore_index=True)
      df["code"] = (
          df["code"].astype(str).str.extract(r"(\d{4})")[0].dropna().astype(str)
      )
      df = df[df["code"].str.len() == 4].drop_duplicates(subset=["code"])
...
def main():
  ...
  if st.sidebar.button(
    ...
    status_text.text("日経225の構成銘柄一覧を取得中...")

    constituents = get_nikkei225_constituents(config["nikkei225_url"])
    if constituents.empty:
      st.error(
          "銘柄一覧の取得に失敗しました。インターネット接続環境をご確認ください。"
      )
      return
    ...
    if not results:
      err_detail = errors[0]["error"] if errors else "取得対象が0件でした。"
      st.error(f"有効なデータを取得できませんでした。詳細: {err_detail}")
      if errors:
        with st.expander("⚠️ エラー詳細ログ"):
