...
# =========================================================
# 学習用インタラクティブローソク足チャート（安全フォーマット化）
# =========================================================
...
if raw_data.empty:
    st.error(f"❌ {display_symbol} のデータを取得できませんでした。ネットワーク接続または銘柄コードを確認してください。")
    st.stop()
...
distance_from_lower = (latest["Close"] / latest["BB_Lower"] - 1) * 100 if latest["BB_Lower"] > 0 else 0.0
...
with tab1:
    ...
    st.caption(f"ローソク足にマウスを乗せると、**その日の「下限まであと何ドルか」「下ヒゲ比率」「注意点・好材料」が瞬時にポップアップ**します。紫色の線＝{mid_trend_period}日移動平均線、オレンジ色の線＝200日移動平均線です。")

    fig = create_learning_candlestick_chart(usable_data, display_symbol, int(mid_trend_period), is_japan_stock)
    st.plotly_chart(fig, use_container_width=True)

    # -----------------------------------------------------
    # グラフ直下の詳細カルテ（日付セレクトボックス：重複完全排除）
    # -----------------------------------------------------
    st.markdown("### 📋 選択した日の詳細カルテ（グラフ直下の分析診断）")
    st.caption("初期値は最新日です。気になる過去の日付を選ぶと、その日の数値と合否理由に切り替わります。")

    # 重複を完全に排除した日付リストを作成（最新日が先頭）
    seen_dates = set()
    recent_date_list = []
    ...
    selected_date_str = st.selectbox(
        "📅 分析したい日付を選択してください（初期値：最新日）",
        options=recent_date_list,
        index=0,
        key=f"target_date_{display_symbol}",
    )

    # 選択日付の安全な取得
    matched_rows = usable_data.loc[usable_data.index.strftime("%Y-%m-%d") == selected_date_str]
    if not matched_rows.empty:
        target_bar = matched_rows.iloc[-1]
    else:
        target_bar = usable_data.iloc[-1]

    close_p = float(target_bar["Close"])
    lower_p = float(target_bar["BB_Lower"])
    diff_lower = close_p - lower_p
    pct_lower = (close_p / lower_p - 1) * 100 if lower_p > 0 else 0.0

    # 200日線の安全取得
    sma200_p = target_bar.get("SMA200", np.nan)
    if pd.notna(sma200_p) and sma200_p > 0:
        diff_200 = close_p - sma200_p
        pct_200 = (close_p / sma200_p - 1) * 100
        sma200_desc = f"株価 {close_p:,.2f}{currency_unit} vs 200日線 {sma200_p:,.2f}{currency_unit}（{diff_200:+,.2f}{currency_unit} / {pct_200:+.1f}%）"
    else:
        sma200_desc = "200日線未算出（データ期間不足のため判定免除）"

    # 中期SMAの安全取得
    mid_sma_p = target_bar.get("Mid_SMA", np.nan)
    if pd.notna(mid_sma_p) and mid_sma_p > 0:
        diff_mid = close_p - mid_sma_p
        pct_mid = (close_p / mid_sma_p - 1) * 100
        mid_sma_desc = f"{mid_trend_period}日線 {mid_sma_p:,.2f}{currency_unit} まで {diff_mid:+,.2f}{currency_unit}（{pct_mid:+.1f}%）"
    else:
    ...
    # 予備のTradingViewチャートは折りたたみに格納して画面をスッキリ整理
    st.markdown("---")
    with st.expander("📈 TradingView風チャート（全体表示）を見る", expanded=False):
