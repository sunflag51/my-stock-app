...
# 【ここを改善】表示用には銘柄名を含んだ selected_option をそのまま使用
display_symbol = selected_option  # 例: "GOOG (アルファベット)" や "9697.T (カプコン)"
_, provider_symbol = normalize_symbol(selected_option)  # データ取得用（例: "GOOG"）
...
with tab1:
    ...
    chart_view = st.radio(
        ...
        horizontal=True,
        key=f"chart_choice_{provider_symbol}",
    )
    ...
    if "Plotly" in chart_view:
        ...
        render_lightweight_chart_safe(
            usable_data, display_symbol, unique_key=f"lw_chart_{provider_symbol}"
        )
...
with tab2:
    ...
    with col_btn2:
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"trade_plan_{provider_symbol}_{datetime.now().strftime('%Y%m%d')}.csv",
            mime="text/csv",
