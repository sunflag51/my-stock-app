...
def normalize_symbol(symbol: str):
    # プルダウンで「7974.T (任天堂)」が選ばれた場合、スペースより前を取得
...
# Session Stateを使って「追加された銘柄」と「選択中の銘柄」を保持する
...
# 「手入力」が選ばれた場合の処理（ここを修正：入力後すぐ表示されるように）
if selected_option == "+ 新しい銘柄を手入力する":
...
if raw_data.empty:
    st.error(
        ...
        "米国株の場合は末尾に「.US」、日本株の場合は「.T」をつけてみてください。"
    )
    if st.button("この銘柄をリストから削除して別の銘柄を選ぶ"):
        if input_symbol in st.session_state.custom_tickers:
            st.session_state.custom_tickers.remove(input_symbol)
            st.session_state.selected_ticker = base_ticker_list[0]
            st.rerun()
    st.stop()
