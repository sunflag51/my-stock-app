...
# 最新判定
# =========================================================
def evaluate_latest(data: pd.DataFrame, score_threshold: float):
    latest = data.iloc[-1]
    ...
    conditions = {
        ...
        "200日線以上": bool(latest["Above_SMA200"]),
...
# =========================================================
# 【復活・修正】累積Rの折れ線グラフ描画機能
# =========================================================
def render_equity_chart(trades_15: pd.DataFrame, trades_20: pd.DataFrame):
    ...
    if not trades_15.empty:
        df15 = trades_15[['決済日', '結果R']].copy()
        df15['RR 1:1.5'] = df15['結果R'].cumsum()
        df15 = df15.set_index('決済日')[['RR 1:1.5']]
        # 同日に複数決済がある場合は最後の値を取る
        df15 = df15.groupby(df15.index).last()
        dfs.append(df15)
        
    if not trades_20.empty:
        df20 = trades_20[['決済日', '結果R']].copy()
        df20['RR 1:2'] = df20['結果R'].cumsum()
        df20 = df20.set_index('決済日')[['RR 1:2']]
        df20 = df20.groupby(df20.index).last()
        dfs.append(df20)
        
    if dfs:
        # 日付で結合
        equity_df = pd.concat(dfs, axis=1)
        # 取引がない期間は横ばい（前の値を引き継ぐ）にし、最初のNaNは0にする
        equity_df = equity_df.ffill().fillna(0)
        
        # エラーが起きないStreamlit標準のラインチャートで描画
...
selected_option = st.sidebar.selectbox("銘柄選択", all_options, index=0)

if selected_option == "+ 新しい銘柄を手入力する":
    new_ticker = st.sidebar.text_input("銘柄コードを入力 (例: 7203.T, MSFT.US)")
    if st.sidebar.button("リストに追加して分析"):
        if new_ticker:
            ...
            st.sidebar.warning("コードを入力してください")
    st.stop()
else:
...
score_threshold = st.sidebar.number_input("条件成立点数", value=7.0, step=0.5)
...
if raw_data.empty:
    ...
    if st.button("この銘柄をリストから削除する"):
        if input_symbol in st.session_state.custom_tickers:
            st.session_state.custom_tickers.remove(input_symbol)
            st.rerun()
    st.stop()

data = add_indicators(raw_data, int(bb_period), float(bb_sigma), int(atr_period), int(swing_lookback))
...
status, status_message, conditions = evaluate_latest(usable_data, float(score_threshold))
...
metric5.metric("条件点数", f"{latest['Score']:.1f}/10")
...
with tab1:
    ...
    st.progress(min(max(float(latest["Score"]) / 10, 0.0), 1.0))
...
with tab3:
    ...
    if st.button("1:1.5と1:2を検証する", type="primary"):
        ...
        st.markdown("### 累積Rの推移")
        render_equity_chart(trades_15, trades_20)
...
- 陽線になり、前の足の終値を上回る
### 3．補助条件
RSI、MACD、出来高、200日線、BB下限の傾きを点数化します。ただし、点数が高くても上昇を保証するものではありません。
### 4．1Rの定義
